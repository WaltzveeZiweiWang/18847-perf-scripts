...
# Hardware types
pc.defineParameter("hardwareTypes",
                   "Optional physical node types separated by commas",
                   portal.ParameterType.STRING, "")

# Variable number of nodes.
pc.defineParameter("nodeCounts", 
                   "Number of nodes per type, separated by commas", 
                   portal.ParameterType.STRING, "1")
...
node_types = [n.strip() for n in params.hardwareTypes.split(',')]
node_counts = [n.strip() for n in params.nodeCounts.split(',')]
for nc in node_counts:
    if not nc.isdigit():
        pc.reportError(portal.ParameterError("Node counts must be numeric.", ["nodeCounts"]))

if (len(node_types) != len(node_counts)):
    pc.reportError(portal.ParameterError("Number of harware types and node counts must match.", ["nodeCounts", "hardwareTypes"]))

for nc in node_counts:
    if nc < 1:
        pc.reportError(portal.ParameterError("Node counts must be greater than zero.", ["nodeCounts"]))
...
for i in range(len(node_types)):
    for j in range(node_counts[i]):
        ...
        # Add to lan
        if total_count > 1:
            iface = node.addInterface("eth1", pg.IPv4Address('10.10.1.{}'.format(node_id+1), '255.255.255.0'))
            lan.addInterface(iface)
            pass
        # Set hardware type
        node.hardware_type = node_types[i]
        # Optional Blockstore
